...
<script>
        ...
        $.ajax({
            ...
            success: function(datas) {
                ...
                if (obj.status == 0) {
                    ...
                    error("feature <?= getLanguage('all', 'exits') ?>");
                    return false;
                }
                else {
                   var id = '<?=$finds->id;?>';	
				...
				if(type != ''){
					window.location = "<?=admin_url();?>feature";
				}
            },
            error: function() {
				$('.loading').hide();
	...
	function addform(){
	   $("#title").val("<?=$finds->title;?>")
		var img = '<?= base_url() ?>files/feature/' + '<?=$finds->image;?>';
		var thumb  = '<?= base_url() ?>files/feature/thumb/' + '<?=$finds->thumb;?>';
		var id = '<?=$finds->id;?>';		
		//$('#id').val(id);
