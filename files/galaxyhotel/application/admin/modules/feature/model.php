<?php
 class FeatureModel extends CI_Model{
	...
	function detail($id){
		$query = $this->model->table('galaxy_feature')
				 ->select('*')
				 ->where('isdelete',0)
				 ->where('id',$id)
				 ->find();
	...
	function getNone(){
		...
		WHERE table_name='galaxy_feature'; 
		";
		//column_name
		$query = $this->model->query($sql)->execute();
		$obj = new stdClass();
	...
	function saves($array){
		 $check = $this->model->table('galaxy_feature')
		 ->select('id')
		 ->where('isdelete',0)
	...
	function edits($array,$id){
		 ...
		 $result = $this->model->table('galaxy_feature')->save($id,$array);	
		 return $result;
	}
