    ...
    <div class="portlet box blue">
        ...
        <div class="portlet-body">
			...
			<div class="row mtop10"> 
				<div class="col-md-4">
					<div class="form-group">
						<label class="control-label col-md-4">Loại web </label>
						<div class="col-md-8" >
							<select name="typeid" id="typeid" class="combos" >
								<option value=""></option>
								<?php foreach ($blogTypes as $item) { ?>
									<option value="<?=$item->id;?>"><?=$item->webtype_name?></option>
								<?php } ?>
							</select>
						</div>
					</div>
...
<script>
        ...
        $.ajax({
            ...
            success: function(datas) {
                ...
                if (obj.status == 0) {
                    ...
                    error("Blog <?= getLanguage('all', 'exits') ?>");
                    return false;
                }
                else {
                   var id = '<?=$finds->id;?>';	
	...
	function addform(){
	   $("#title").val("<?=$finds->title;?>")
		var img = '<?= base_url() ?>files/web/' + '<?=$finds->image;?>';
		var thumb  = '<?= base_url() ?>files/web/thumb/' + '<?=$finds->thumb;?>';
		var id = '<?=$finds->id;?>';		
		//$('#id').val(id);
