 ...
 class UserModel extends CI_Model{
	...
	function getList($search,$page,$numrows){
		$sql = " SELECT u.id,u.username, u.fullname, u.groupid, u.image, 
				...
				FROM galaxy_users AS u 
				LEFT JOIN galaxy_groups AS g ON g.id = u.groupid
				WHERE u.isdelete = 0 ";
	...
	function saves($array){
		 $check = $this->model->table('galaxy_users')
		 ->select('id')
		 ->where('isdelete',0)
	...
	function edits($array,$id){
		 ...
		 $result = $this->model->table('galaxy_users')->save($id,$array);	
		 return $result;
	}
