...
class Blog extends CI_Controller {
    ...
    function save() {
		...
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
            ...
            $resize = $this->resizeImg($image_data,400,400);
        }
        $login = $this->login;
    ...
    function edit() {
		...
		$finds = $this->model->table('galaxy_blog')
					  ...
					  ->find();
        if (isset($_FILES['userfile']) && $_FILES['userfile']['name'] != "") {
			if(file_exists('files/blog/'.$finds->image)){
				unlink('files/blog/'.$finds->image);
			}
			$imge_name = $_FILES['userfile']['name'];
            $this->upload->initialize($this->set_upload_options());
            $image_data = $this->upload->do_upload('userfile', $imge_name); //Ten hinh 
		...
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/blog/thumb/'.$finds->thumb)){
...
//print_r($array);exit;
    ...
    function resizeImg($image_data,$width='',$height='') {
        ...
        $configz['source_image'] = './files/blog/thumb/' . $image_data;
        $configz['new_image'] = './files/blog/thumb/' . $image_data;
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/blog/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(file_exists('files/blog/'.$finds->image) && !empty($finds->image)){
			unlink('files/blog/'.$finds->image);
		}
