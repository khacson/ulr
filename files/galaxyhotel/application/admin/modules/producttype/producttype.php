...
class Producttype extends CI_Controller {
    ...
    function edit() {
		...
		$finds = $this->model->table('galaxy_producttype')
					  ->select('image,thumb')
					  ->where('id',$id)
					  ->find();
		$login = $this->login;
				...
				if($i==0){
					if(!empty($finds->image)){
						...
						foreach($arrImg as $k=>$v){
							if(file_exists('files/producttype/'.$v)){
								unlink('files/producttype/'.$v);
							}
						}
					}
				}
		...
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/producttype/thumb/'.$finds->thumb) && !empty($finds->thumb)){
				unlink('files/producttype/thumb/'.$finds->thumb);
			}
			$imge_name = $_FILES['userfile2']['name'];
            $this->upload->initialize($this->set_upload_options2());
            $image_data = $this->upload->do_upload('userfile2', $imge_name); //Ten hinh 
	...
	function deleteimg(){
		...
		if(!empty($finds->image)){
			...
			$this->model->table('galaxy_producttype')->save($id,$array);
			unlink('files/producttype/'.$img);
		}	
	}
    function resizeImg($image_data,$width='',$height='') {
        ...
        $configz['source_image'] = './files/producttype/thumb/' . $image_data;
        $configz['new_image'] = './files/producttype/thumb/' . $image_data;
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/producttype/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(!empty($finds->image)){
			$arrImg = explode(';',$finds->image);
			foreach($arrImg as $k=>$v){
				if(file_exists('files/producttype/'.$v) && !empty($v)){
					unlink('files/producttype/'.$v);
				}
			}
		}	
		...
		$this->model->table('galaxy_producttype')->where("id in ($id)")->delete();	
		$result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
