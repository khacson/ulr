...
class Seoweb extends CI_Controller {
    ...
    function _view() {
        ...
        $data->seowebTypes = $this->model->seowebTypes('');
    ...
    function save() {
        ...
        $login = $this->login;
		$array['friendlyurl'] = $this->admin->friendlyURL($array['meta_title']);
    ...
    function edit() {
        ...
        $login = $this->login;
		$finds = $this->model->table('galaxy_seoweb')
					  ->select('image')
					  ->where('id',$id)
					  ->find();
		if (isset($_FILES['userfile']) && $_FILES['userfile']['name'] != "") {
			if(file_exists('files/seoweb/'.$finds->image)){
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/seoweb/';
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/seoweb/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		$this->model->table('galaxy_seoweb')->where("id in ($id)")->delete();	
