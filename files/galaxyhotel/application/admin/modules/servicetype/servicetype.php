...
class Servicetype extends CI_Controller {
    ...
    function save() {
        ...
        $login = $this->login;
        $array['friendlyurl'] = $this->admin->friendlyURL($array['type_name']);
    ...
    function deletes() {
        ...
        $array['isdelete'] = 1;
		$this->model->table('galaxy_servicetype')->where('id',$id)->delete();
        $result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
    ...
    function isHome(){
		...
		$this->model->table('galaxy_servicetype')->save($id,$array);	
	}
	function ordering(){
		$array = array();
		$id = $this->input->post('id');
	...
	function getCatalog(){
		$catalogs = $this->model->getCatalog('');	
		$html = "";
		$html.= '<select id="parentid" name="parentid" class="combos">';
		$html.= '<option value="0"></option><option value="0">Cấp 1</option>';
		foreach($catalogs as $item){
			$html.= '<option value="'.$item->id .'">'.$item->type_name .'</option>';				
		}
		$html.= '</select>';
		echo $html;
	}
