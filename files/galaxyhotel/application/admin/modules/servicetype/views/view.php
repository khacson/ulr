            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        <label class="control-label col-md-4">Loại sản phẩm (<span class="red">*</span>)</label>
                        <div class="col-md-8">
                            <input type="text" name="type_name" id="type_name" class="searchs form-control" />
                        </div>
                    </div>
                </div>
				<div class="col-md-4">
                    <div class="form-group">
                        <label class="control-label col-md-4">Cấp cha</label>
                        <div class="col-md-8" id="loadParent">
							<select id="parentid" name="parentid" class="combos">
...
<div class="portlet box blue">
    <div class="portlet-title">
        <div class="caption" style="margin-top:4px;">
            <i>Có <span class='viewtotal'>0</span> loại sản phẩm</i>
        </div>
        <div class="tools">
                    ...
                    <div id="tHeader">    	
                        <table id="tbheader" width="100%" cellspacing="0" border="1" >
                            ...
                            <tr>
                                ...
                                <th>STT</th>
                                <th id="ord_type_name">Loại sản phẩm</th>
                                <th>Hình ảnh</th>
								<th>Cấp cha</th>
                                <th id="ord_ishome">Hiện thị</th>
...
<script>
    ...
    $(function() {
        ...
        $('#edit').click(function() {
            var id = $('#id').val();
            if (id == '') {
                warning('Vui lòng chọn loại sản phẩm để sửa');
                return false;
            }
            save('edit', id);
        });
        $('#delete').click(function() {
            $.msgBox({
                title: 'Message',
                type: 'error',
                content: 'Bạn muốn xóa Loại sản phẩm?',
	...
	function getParent(){
		 $.ajax({
            url: controller + 'getCatalog',
            type: 'POST',
            async: false,
            data: {},
            success: function(datas) {
				$('#loadParent').html(datas);
				$('#parentid').multipleSelect({
					filter: true,
					placeholder:"Chọn cấp cha",
					single: true
        ...
        $.ajax({
            ...
            success: function(datas) {
                ...
                if (obj.status == 0) {
                    ...
                    error("Loại sản phẩm <?= getLanguage('all', 'exits') ?>");
                    return false;
                }
                else {
					if (id != '') {
    ...
    function funcList(obj) {
        $('.edit').each(function(e) {
            $(this).click(function() {
                var type_name = $('.type_name').eq(e).html().trim();
                var id = $(this).attr('id');
				var parentid = $(this).attr('parentid');
				var meta_title = $('.meta_title').eq(e).html();
				var meta_keyword = $('.meta_keyword').eq(e).html();
				var mete_description = $('.mete_description').eq(e).html();
                $('#id').val(id);
                $('#type_name').val(type_name);
				$('#meta_title').val(meta_title);
				$('#meta_keyword').val(meta_keyword);
				$('#mete_description').val(mete_description);
