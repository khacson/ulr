...
//print_r($array);exit;
    ...
    function resizeImg($image_data,$width='',$height='') {
        ...
        $configz['source_image'] = './files/picture/thumb/' . $image_data;
        $configz['new_image'] = './files/picture/thumb/' . $image_data;
    ...
    function deletes() {
		...
		if(file_exists('files/picture/'.$finds->image) && !empty($finds->image)){
			unlink('files/picture/'.$finds->image);
		}
