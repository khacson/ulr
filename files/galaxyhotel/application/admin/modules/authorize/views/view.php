...
<div class="form-group" style="margin-top
    ...
    <div class="col-md-4">
        <img  title="Create new captcha"  src="<?= admin_url() ?>authorize/captcha/543534.html" id="icaptcha">
    </div>
    <div class="col-md-2" style="text-align:right;">
        <img id="reload" title="Create another code" style="cursor:pointer; margin-left:5px; margin-top:3px; width:28px;" align="absmiddle" src="<?= url_tmpl(); ?>images/reload.png" />
    </div>
</div>
<div class="form-group" >
    <div class="col-md-12" style="margin-top:0px; padding-left:25px;  padding-right:0px; ">
...
<div style="color:#999; font-site:12px; margin-top:10px;">Version 1.0.1</div>
<input id="flag" value="0" type="hidden" />
<script type="text/javascript">
            var controller = '<?= admin_url() . $routes; ?>/';
            $(function() {
            ...
            function login() {
                if($("#flag").val() == 1){
                    ...
                    if (verification == '') {
                        error("Captcha can not be empty.");
                        $(".loading").hide();
                        $("#flag").val(1);
                        return false;
                    }
                    ...
                    $.ajax({
                        ...
                        success: function(datas) {
                            ...
                            $("#token").val(obj.token);
                            if (obj.status == 1) {
                                window.location = "<?= admin_url() ?>home.html";
                                $(".loading").hide();
                            }
                            else {
                                var id = randomNumberFromRange(100, 1000);
                                $("#flag").val(1);
                                $("#icaptcha").attr("src", "<?= admin_url() ?>authorize/captcha/" + id + ".html");
                                error("Username, password or captcha incorrect.");
                                $(".loading").hide();
                            }
                        },
                        error: function() {
