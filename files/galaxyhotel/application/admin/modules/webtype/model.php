<?php
 class WebtypeModel extends CI_Model{
	...
	function getCatalog(){
		$query = $this->model->table('galaxy_webtype')
	...
	function saves($array){
		 $check = $this->model->table('galaxy_webtype')
		 ->select('id')
		 ->where('isdelete',0)
	...
	function edits($array,$id){
		 ...
		 unset($array['fromdate']);
                 unset($array['todate']);
		 $result = $this->model->table('galaxy_webtype')->save($id,$array);	
		 return $result;
	}
