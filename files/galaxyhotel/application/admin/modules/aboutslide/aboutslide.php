...
class Aboutslide extends CI_Controller {
    ...
    function save() {
        ...
        if (isset($_FILES['userfile']) && $_FILES['userfile']['name'] != "") {
            ...
            /*$data = getimagesize(base_url().'files/aboutslide/'.$image_data);
            $width = $data[0];
            $height = $data[1];
            if($width < 100){
                $result['status'] = 2;
    ...
    function resizeImg($image_data) {
        ...
        $configz['source_image'] = './files/aboutslide/' . $image_data;
        $configz['new_image'] = './files/aboutslide/' . $image_data;
    ...
    function deletes() {
        ...
        $array['isdelete'] = 1;
        //$this->model->table('galaxy_slide_top')->save($id, $array);
		$sql=" UPDATE `galaxy_slide_top` SET `dateupdate` = '".$array['dateupdate']."', `userupdate` = '".$login->username."', `isdelete` = 1
		WHERE `id` in (".$id.")";       
	...
	function ordering(){
		...
		$this->model->table('galaxy_slide_about')->save($id,$array);	
	}
}