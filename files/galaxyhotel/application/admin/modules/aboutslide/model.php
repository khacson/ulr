<?php
 class AboutslideModel extends CI_Model{
	...
	function getList($search,$page,$numrows){
		$sql = "SELECT id,img,slide_name,description,url,datecreate,usercreate, ordering
                        FROM galaxy_slide_about
                        WHERE isdelete = 0";
	...
	function saves($array){
		 $check = $this->model->table('galaxy_slide_about')
		 ->select('id')
		 ->where('isdelete',0)
		 ...
		 $result = $this->model
						->table('galaxy_slide_about')
						->insert($array);	
		 return $result;
	}
	function edits($array,$id){
		 $result = $this->model->table('galaxy_slide_about')->save($id,$array);	
		 return $result;
	}
