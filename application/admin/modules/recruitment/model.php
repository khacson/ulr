<?php
 class recruitmentModel extends CI_Model{
	...
	function detail($id){
		$query = $this->model->table('mec_recruitment')
				 ->select('*')
				 ->where('isdelete',0)
				 ->where('id',$id)
				 ->find();
	...
	function getNone(){
		...
		WHERE table_name='mec_recruitment'; 
		";
		$query = $this->model->query($sql)->execute();
		$obj = new stdClass();
	...
	function saves($array){
		...
		$result = $this->model->table('mec_recruitment')
						->insert($array);	
		return $result;
	}
	function edits($array,$id){
		$array['language'] = str_replace('"','',$array['language']);
		$result = $this->model->table('mec_recruitment')->save($id,$array);	
		return $result;
	}
