...
<div class="loading" style="display: none;">
    ...
    <div class="blockUI blockMsg blockElement" style="width: 30%;position: absolute;top: 15%;left:35%;text-align: center;">
        <img src="<?= url_tmpl() ?>img/ajax_loader.gif" style="z-index: 29999;position: absolute;"/>
    </div>
</div> 
<style>
	.control-label {
...
<script>
        ...
        $.ajax({
            ...
            success: function(datas) {
                ...
                if (obj.status == 0) {
                    ...
                    error("recruitment <?= getLanguage('all', 'exits') ?>");
                    return false;
                }
                else {
                   var id = '<?=$finds->id;?>';	
				...
				if(type != ''){
					window.location = "<?=admin_url();?>recruitment";
				}
            },
            error: function() {
				$('.loading').hide();
	...
	function addform(){
	   $("#title").val("<?=$finds->title;?>")
		...
		var thumb  = '<?= base_url() ?>files/recruitment/thumb/' + '<?=$finds->thumb;?>';
		var id = '<?=$finds->id;?>';		
		//$('#id').val(id);
		...
		if(id!=''){
			...
			<?php foreach($arrImg as $k=>$val){?>
				<?php if(!empty($val)){?>
				var img = '<?= base_url() ?>files/recruitment/' + '<?=$val;?>';
