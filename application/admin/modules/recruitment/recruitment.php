...
class Recruitment extends CI_Controller {
    ...
    function getList() {
        ...
        $data->datas = $query;
        $page_view = $this->admin->pagination($count, $numrows, 5, 'recruitment/', $page);
    ...
    function edit() {
		...
		$finds = $this->model->table('mec_recruitment')
					  ->select('image,thumb')
					  ->where('id',$id)
					  ->find();
		$login = $this->login;
				...
				if($i==0){
					if(!empty($finds->image)){
						...
						foreach($arrImg as $k=>$v){
							if(file_exists('files/recruitment/'.$v)){
								unlink('files/recruitment/'.$v);
							}
						}
					}
				}
		...
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/recruitment/thumb/'.$finds->thumb) && !empty($finds->thumb)){
				unlink('files/recruitment/thumb/'.$finds->thumb);
			}
			$imge_name = $_FILES['userfile2']['name'];
            $this->upload->initialize($this->set_upload_options2());
            $image_data = $this->upload->do_upload('userfile2', $imge_name); //Ten hinh 
...
//print_r($array);exit;
	...
	function deleteimg(){
		...
		if(!empty($finds->image)){
			...
			$this->model->table('mec_recruitment')->save($id,$array);
			unlink('files/recruitment/'.$img);
		}	
	}
    function resizeImg($image_data,$width='',$height='') {
        ...
        $configz['source_image'] = './files/recruitment/thumb/' . $image_data;
        $configz['new_image'] = './files/recruitment/thumb/' . $image_data;
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/recruitment/';
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/recruitment/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(!empty($finds->image)){
			$arrImg = explode(';',$finds->image);
			foreach($arrImg as $k=>$v){
				if(file_exists('files/recruitment/'.$v) && !empty($v)){
					unlink('files/recruitment/'.$v);
				}
			}
		}	
		...
		$this->model->table('mec_recruitment')->where("id in ($id)")->delete();	
		$result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
