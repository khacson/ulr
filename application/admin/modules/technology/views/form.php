...
<script>
        ...
        $.ajax({
            ...
            success: function(datas) {
                ...
                if (obj.status == 0) {
                    ...
                    error("technology <?= getLanguage('all', 'exits') ?>");
                    return false;
                }
                else {
                   var id = '<?=$finds->id;?>';	
				...
				if(type != ''){
					window.location = "<?=admin_url();?>technology";
				}
            },
            error: function() {
				$('.loading').hide();
	...
	function addform(){
	   $("#title").val("<?=$finds->title;?>")
		...
		var thumb  = '<?= base_url() ?>files/technology/thumb/' + '<?=$finds->thumb;?>';
		var id = '<?=$finds->id;?>';		
		//$('#id').val(id);
		...
		if(id!=''){
			...
			<?php foreach($arrImg as $k=>$val){?>
				<?php if(!empty($val)){?>
				var img = '<?= base_url() ?>files/technology/' + '<?=$val;?>';
