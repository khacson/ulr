    ...
    <div class="portlet box blue">
        <div class="portlet-title">
            ...
            <div class="tools">
				<ul class="button-group pull-right" style="margin-top:-3px; margin-bottom:5px;">   
							<li id="">
							   <a href="<?=admin_url();?>designpicture">
        ...
        <div class="portlet-body">
			...
			<div class="row mtop10"> 
				<div class="col-md-4">
					<div class="form-group">
						<label class="control-label col-md-4">Thiết kế </label>
						<div class="col-md-8" >
							<select name="typeid" id="typeid" class="combos" >
								<option value=""></option>
								<?php foreach ($designs as $item) { ?>
									<option <?php if($item->id == $finds->typeid){?> selected <?php }?> value="<?=$item->id;?>"><?=$item->language;?> - <?=$item->title;?></option>
								<?php } ?>
							</select>
						</div>
					</div>
...
<script>
    ...
    function save(func, id, type) {
        ...
        $.ajax({
            ...
            success: function(datas) {
				...
				if(type != ''){
					window.location = "<?=admin_url();?>designpicture";
				}
            },
            error: function() {
				$('.loading').hide();
	...
	function addform(){
	   $("#title").val("<?=$finds->title;?>")
		var img = '<?= base_url() ?>files/designpicture/' + '<?=$finds->image;?>';
		var id = '<?=$finds->id;?>';		
		//$('#id').val(id);
