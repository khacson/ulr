 ...
 * @copyright mec_
 */
class Designpicture extends CI_Controller {
    ...
    function _view() {
        ...
        //$data->Designs = $this->model->blogDesign();
		$data->designs = $this->model->blogDesign();
    ...
    function edit() {
        ...
        $login = $this->login;
		$finds = $this->model->table('mec_picture_design')
					  ->select('image,thumb')
					  ->where('id',$id)
					  ->find();
        if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/designpicture/'.$finds->image)){
				unlink('files/designpicture/'.$finds->image);
			}
			$imge_name = $_FILES['userfile2']['name'];
            $this->upload->initialize($this->set_upload_options());
            $image_data = $this->upload->do_upload('userfile2', $imge_name); //Ten hinh 
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/designpicture/';
	...
	private function set_upload_options2() {
        $config = array();
		$config['allowed_types'] = 'jpg|jpeg|gif|png|mp4|avi|flv|wmv|mov';
        $config['upload_path'] = './files/designpicture/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(file_exists('files/designpicture/'.$finds->image) && !empty($finds->image)){
			unlink('files/designpicture/'.$finds->image);
		}
		if(file_exists('files/designpicture/thumb/'.$finds->thumb) && !empty($finds->thumb)){
			unlink('files/designpicture/thumb/'.$finds->thumb);	
		}
		$this->model->table('mec_picture_design')->where("id in ($id)")->delete();	
	...
	function isshow(){
		...
		$this->model->table('mec_picture_design')->save($id,$array);	
	}
	function ordering(){
		$array = array();
		$id = $this->input->post('id');
