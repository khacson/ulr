...
class Employee extends CI_Controller {
    ...
    function edit() {
        ...
        $login = $this->login;
		$finds = $this->model->table('ndnt_employee')
					  ->select('image')
					  ->where('id',$id)
					  ->find();
        if (isset($_FILES['userfile']) && $_FILES['userfile']['name'] != "") {
			if(file_exists('files/employee/'.$finds->image)){
				unlink('files/employee/'.$finds->image);
			}
			$imge_name = $_FILES['userfile']['name'];
            $this->upload->initialize($this->set_upload_options());
            $image_data = $this->upload->do_upload('userfile', $imge_name); //Ten hinh 
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/employee/';
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/employee/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(file_exists('files/employee/'.$finds->image) && !empty($finds->image)){
			unlink('files/employee/'.$finds->image);
		}
		$this->model->table('ndnt_employee')->where("id in ($id)")->delete();	
	...
	function isshow(){
		...
		$this->model->table('ndnt_employee')->save($id,$array);	
	}
	function ordering(){
		$array = array();
		$id = $this->input->post('id');
