	...
	<tr class="content edit" img = "<?=$item->thumb;?>" 
		...
		<td class="title">
		 <?php if (isset($permission['edit'])) { ?>
		   <a href="<?=admin_url();?>nhadatban/form/<?=$item->id;?>"><?=$item->title;?></a>
		 <?php }else{?>
			<?=$item->title;?>
		 <?php }?>
		</td>
		...
		<td class="address"></td>
		<td class="nhadatbantype_name"><?=$item->catalog_name;?></td>
        <td class="text-center"><img src="<?php echo base_url().'files/nhadatban/thumb/'.$item->thumb;?>" alt="img" height="45" width="60"></td>	
