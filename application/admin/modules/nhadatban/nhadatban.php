 ...
 * @copyright 2015
 */
class Nhadatban extends CI_Controller {
    ...
    function _view() {
        ...
        $data->catalogs = $this->model->getCatalogs();
    ...
    function edit() {
		...
		$finds = $this->model->table('ndnt_product')
					  ->select('image,thumb')
					  ->where('id',$id)
					  ->find();
		$login = $this->login;
...
//print_r($array);exit;
	...
	function deleteimg(){
		...
		if(!empty($finds->image)){
			...
			$this->model->table('ndnt_product')->save($id,$array);
    ...
    function deletes() {
		...
		$this->model->table('ndnt_product')->where("id in ($id)")->delete();	
		$result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
