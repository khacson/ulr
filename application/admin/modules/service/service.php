...
class Service extends CI_Controller {
    ...
    function _view() {
		...
		$arrP = array();
		$arr = array();
		foreach($data->serviceTypes as $item){
			if($item->parentid == 0){
				$arrP[$item->id] = $item->servicetype_name;
			}
			else{
				$arr[$item->parentid][$item->id] = $item;
			}
    ...
    function save() {
        ...
        $array['price_type'] = $this->input->post('price_type');
		...
		if(!empty($car_images)){
			$array['image'] = substr($car_images,1);
		}
		...
		$array['maps_address'] = $this->input->post('maps_address');
    ...
    function edit() {
		...
		$finds = $this->model->table('ndnt_service')
					  ->select('image,thumb')
					  ->where('id',$id)
					  ->find();
		$login = $this->login;
				...
				if($i==0){
					if(!empty($finds->image)){
						...
						foreach($arrImg as $k=>$v){
							if(file_exists('files/service/'.$v)){
								unlink('files/service/'.$v);
							}
						}
					}
				}
		...
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/service/thumb/'.$finds->thumb) && !empty($finds->thumb)){
				unlink('files/service/thumb/'.$finds->thumb);
			}
			$imge_name = $_FILES['userfile2']['name'];
            $this->upload->initialize($this->set_upload_options2());
            $image_data = $this->upload->do_upload('userfile2', $imge_name); //Ten hinh 
...
//print_r($array);exit;
	...
	function deleteimg(){
		...
		if(!empty($finds->image)){
			...
			$this->model->table('ndnt_service')->save($id,$array);
			unlink('files/service/'.$img);
		}	
	}
    ...
    function deletes() {
		...
		if(!empty($finds->image)){
			$arrImg = explode(';',$finds->image);
			foreach($arrImg as $k=>$v){
				if(file_exists('files/service/'.$v) && !empty($v)){
					unlink('files/service/'.$v);
				}
			}
		}	
		...
		$this->model->table('ndnt_service')->where("id in ($id)")->delete();	
		$result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
