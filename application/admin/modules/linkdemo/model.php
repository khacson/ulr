<?php
 class LinkdemoModel extends CI_Model{
	...
	function detail($id){
		$query = $this->model->table('ndnt_linkdemo')
				 ->select('*')
				 ->where('isdelete',0)
				 ->where('id',$id)
				 ->find();
	...
	function getNone(){
		...
		WHERE table_name='ndnt_linkdemo'; 
		";
		//column_name
		$query = $this->model->query($sql)->execute();
		$obj = new stdClass();
	...
	function saves($array){
		 $check = $this->model->table('ndnt_linkdemo')
		 ->select('id')
		 ->where('isdelete',0)
	...
	function edits($array,$id){
		 ...
		 $result = $this->model->table('ndnt_linkdemo')->save($id,$array);	
		 return $result;
	}
