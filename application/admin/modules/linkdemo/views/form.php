            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="form-group">
                        ...
                        <div class="col-md-10">
                            <input style="margin-left:-3px;" type="text" name="title" id="title" value="<?=$finds->title;?>" class="searchs form-control" />
                        </div>
                    </div>
                </div>  
				 <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <div class="col-md-8">
                            <input  type="text" name="url" id="url" value="<?=$finds->url;?>" class="searchs form-control" />
                        </div>
                    </div>
                </div>  
            </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function(datas) {
                ...
                if (obj.status == 0) {
                    ...
                    error("linkdemo <?= getLanguage('all', 'exits') ?>");
                    return false;
                }
                else {
                   var id = '<?=$finds->id;?>';	
				...
				if(type != ''){
					window.location = "<?=admin_url();?>linkdemo";
				}
            },
            error: function() {
				$('.loading').hide();
	...
	function addform(){
	   $("#title").val("<?=$finds->title;?>")
		var img = '<?= base_url() ?>files/linkdemo/' + '<?=$finds->image;?>';
		var thumb  = '<?= base_url() ?>files/linkdemo/thumb/' + '<?=$finds->thumb;?>';
		var id = '<?=$finds->id;?>';		
		//$('#id').val(id);
