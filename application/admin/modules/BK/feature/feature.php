...
class Feature extends CI_Controller {
    ...
    function edit() {
		...
		$finds = $this->model->table('mec_feature')
					  ...
					  ->find();
        if (isset($_FILES['userfile']) && $_FILES['userfile']['name'] != "") {
			if(file_exists('files/feature/'.$finds->image)){
				unlink('files/feature/'.$finds->image);
			}
			$imge_name = $_FILES['userfile']['name'];
            $this->upload->initialize($this->set_upload_options());
            $image_data = $this->upload->do_upload('userfile', $imge_name); //Ten hinh 
		...
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/feature/thumb/'.$finds->thumb)){
				unlink('files/feature/thumb/'.$finds->thumb);
			}
			$imge_name = $_FILES['userfile2']['name'];
            $this->upload->initialize($this->set_upload_options2());
            $image_data = $this->upload->do_upload('userfile2', $imge_name); //Ten hinh 
...
//print_r($array);exit;
    ...
    function resizeImg($image_data,$width='',$height='') {
        ...
        $configz['source_image'] = './files/feature/thumb/' . $image_data;
        $configz['new_image'] = './files/feature/thumb/' . $image_data;
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/feature/';
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/feature/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(file_exists('files/feature/'.$finds->image) && !empty($finds->image)){
			unlink('files/feature/'.$finds->image);
		}
		if(file_exists('files/feature/thumb/'.$finds->thumb) && !empty($finds->thumb)){
			unlink('files/feature/thumb/'.$finds->thumb);	
		}
