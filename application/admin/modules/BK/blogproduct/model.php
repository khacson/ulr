<?php
 class BlogproductModel extends CI_Model{
	...
	function detail($id){
		$query = $this->model->table('mec_blogproduct')
				 ->select('*')
				 ->where('isdelete',0)
				 ->where('id',$id)
				 ->find();
	...
	function getNone(){
		...
		WHERE table_name='mec_blogproduct'; 
		";
		//column_name
		$query = $this->model->query($sql)->execute();
		$obj = new stdClass();
	...
	function saves($array){
		 $check = $this->model->table('mec_blogproduct')
		 ->select('id')
		 ->where('isdelete',0)
	...
	function edits($array,$id){
		 ...
		 $result = $this->model->table('mec_blogproduct')->save($id,$array);	
		 return $result;
	}
