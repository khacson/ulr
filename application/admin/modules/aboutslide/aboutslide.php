 ...
 * @copyright 2018
 */
class Aboutslide extends CI_Controller {
    ...
    function getList() {
        ...
        $data->datas = $query;
        $page_view = $this->admin->pagination($count, $numrows, 5, 'aboutslide/', $page);
    ...
    function edit() {
		...
		$finds = $this->model->table('mec_slide_about')
					  ->select('image,thumb')
					  ->where('id',$id)
					  ->find();
		$login = $this->login;
				...
				if($i==0){
					if(!empty($finds->image)){
						...
						foreach($arrImg as $k=>$v){
							if(file_exists('files/aboutslide/'.$v)){
								unlink('files/aboutslide/'.$v);
							}
						}
					}
				}
		...
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/aboutslide/thumb/'.$finds->thumb) && !empty($finds->thumb)){
				unlink('files/aboutslide/thumb/'.$finds->thumb);
			}
			$imge_name = $_FILES['userfile2']['name'];
            $this->upload->initialize($this->set_upload_options2());
            $image_data = $this->upload->do_upload('userfile2', $imge_name); //Ten hinh 
...
//print_r($array);exit;
	...
	function deleteimg(){
		...
		if(!empty($finds->image)){
			...
			$this->model->table('mec_slide_about')->save($id,$array);
			unlink('files/aboutslide/'.$img);
		}	
	}
    function resizeImg($image_data,$width='',$height='') {
        ...
        $configz['source_image'] = './files/aboutslide/thumb/' . $image_data;
        $configz['new_image'] = './files/aboutslide/thumb/' . $image_data;
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/aboutslide/';
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/aboutslide/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(!empty($finds->image)){
			$arrImg = explode(';',$finds->image);
			foreach($arrImg as $k=>$v){
				if(file_exists('files/aboutslide/'.$v) && !empty($v)){
					unlink('files/aboutslide/'.$v);
				}
			}
		}	
		...
		$this->model->table('mec_slide_about')->where("id in ($id)")->delete();	
		$result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
