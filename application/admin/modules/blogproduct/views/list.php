	...
	<tr class="content edit" img = "<?=$item->img;?>" id="<?=$item->id; ?>" >
		...
		<td class="title">
		 <?php if (isset($permission['edit'])) { ?>
		   <a href="<?=admin_url();?>blogproduct/form/<?=$item->id;?>"><?=$item->title;?></a>
		 <?php }else{?>
			<?=$item->title;?>
		 <?php }?>
		</td>
		<td class="productname"><?=$item->productname;?></td>
        <td class="text-center"><img src="<?php echo base_url().'files/blogproduct/'.$item->image?>" alt="img" height="45" width="60"></td>	
