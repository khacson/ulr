 ...
 */
 class AuthorizeModel extends CI_Model{
	...
	function login($u, $p = null) {
		$query = $this->model->table('vland_users')
		->select("vland_users.*,vland_groups.params")
		->join('vland_groups', 'vland_groups.id = vland_users.groupid', 'inner')
		->where('vland_users.isdelete',0)	
		->where('vland_groups.isdelete',0)	
		->where('vland_users.username',$u)
		->find();
		return $query;
	}
	function getListMenu(){
		$menu = $this->model->table('vland_menus')
							->select('name,route')
							->where('isdelete',0)
							->where('route <>','')
							->where('route <>','#')
	...
	function insertTimeLog($uid , $address, $GMTTime){
		...
		$data['username'] = $uid;	
		$id = $this->model->table('vland_time_login')->save('', $data);
		return $id;
	}
