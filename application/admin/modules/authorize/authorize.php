...
class authorize extends CI_Controller {
	...
	function _view(){
		...
		if(isset($_COOKIE['sellmec_user'])){
			$data->username = $_COOKIE['sellmec_user'];
		}
		else{
			$data->username = '';
		}
		if(isset($_COOKIE['sellmec_pass'])){
			$data->password = $_COOKIE['sellmec_pass'];
		}
		else{
			$data->password = '';
		}
	...
	function login(){
		...
		if (count((array)$login) > 0) {
			if ($pass == $login->password){ // compare password success
				...
				$result['status'] = 1;
				if($remember == 1){
					setcookie('sellmec_user',$u, time() + (86400 * 7),"/"); 
					setcookie('sellmec_pass',$p, time() + (86400 * 7),"/"); 
				}
				else{
					setcookie('sellmec_user','', time() + (86400 * 7),"/"); 
					setcookie('sellmec_pass','', time() + (86400 * 7),"/"); 
				}
				$result['token'] = $this->security->get_csrf_hash();
