...
<script>
        ...
        $.ajax({
            ...
            success: function(datas) {
                ...
                if (obj.status == 0) {
                    ...
                    error("design <?= getLanguage('all', 'exits') ?>");
                    return false;
                }
                else {
                   var id = '<?=$finds->id;?>';	
				...
				if(type != ''){
					window.location = "<?=admin_url();?>design";
				}
            },
            error: function() {
				$('.loading').hide();
	...
	function addform(){
	   $("#title").val("<?=$finds->title;?>")
		var thumb  = '<?= base_url() ?>files/design/thumb/' + '<?=$finds->thumb;?>';
		var id = '<?=$finds->id;?>';		
		...
		if(id!=''){
			...
			<?php foreach($arrImg as $k=>$val){?>
				<?php if(!empty($val)){?>
				var img = '<?= base_url() ?>files/design/' + '<?=$val;?>';
