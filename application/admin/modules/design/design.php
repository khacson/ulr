...
class Design extends CI_Controller {
    ...
    function getList() {
        ...
        $data->datas = $query;
        $page_view = $this->admin->pagination($count, $numrows, 5, 'design/', $page);
    ...
    function edit() {
		...
		$finds = $this->model->table('mec_design')
					  ->select('image,thumb')
					  ->where('id',$id)
					  ->find();
		$login = $this->login;
				...
				if($i==0){
					if(!empty($finds->image)){
						...
						foreach($arrImg as $k=>$v){
							if(file_exists('files/design/'.$v)){
								unlink('files/design/'.$v);
							}
						}
					}
				}
		...
		//print_r($_FILES); exit;
		if (isset($_FILES['userfile2']) && $_FILES['userfile2']['name'] != "") {
			if(file_exists('files/design/thumb/'.$finds->thumb) && !empty($finds->thumb)){
				unlink('files/design/thumb/'.$finds->thumb);
			}
			$imge_name = $_FILES['userfile2']['name'];
            $this->upload->initialize($this->set_upload_options2());
            $image_data = $this->upload->do_upload('userfile2', $imge_name); //Ten hinh 
...
//print_r($array);exit;
	...
	function deleteimg(){
		...
		if(!empty($finds->image)){
			...
			$this->model->table('mec_design')->save($id,$array);
			unlink('files/design/'.$img);
		}	
	}
    function resizeImg($image_data,$width='',$height='') {
        ...
        $configz['source_image'] = './files/design/thumb/' . $image_data;
        $configz['new_image'] = './files/design/thumb/' . $image_data;
    ...
    private function set_upload_options() {
        ...
        $config['upload_path'] = './files/design/';
	...
	private function set_upload_options2() {
        $config = array();
        $config['allowed_types'] = 'jpg|jpeg|gif|png';
        $config['upload_path'] = './files/design/thumb/';
        $config['encrypt_nam'] = 'TRUE';
        $config['remove_spaces'] = TRUE;
        //$config['max_size'] = 0024;
        return $config;
    ...
    function deletes() {
		...
		if(!empty($finds->image)){
			$arrImg = explode(';',$finds->image);
			foreach($arrImg as $k=>$v){
				if(file_exists('files/design/'.$v) && !empty($v)){
					unlink('files/design/'.$v);
				}
			}
		}	
		...
		$this->model->table('mec_design')->where("id in ($id)")->delete();	
		$result['status'] = 1;
        $result['csrfHash'] = $token;
        echo json_encode($result);
    }
